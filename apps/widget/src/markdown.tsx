    ...
    boldSplit.forEach((seg, bi) => {
      ...
      italSplit.forEach((s2, ii) => {
        ...
        while ((m = URL_RE.exec(s2)) !== null) {
          if (m.index > last) nodes.push(<span key={`${keyPrefix}-t${ci}${bi}${ii}-${mi++}`}>{s2.slice(last, m.index)}</span>);
          nodes.push(
            <a key={`${keyPrefix}-l${ci}${bi}${ii}-${mi++}`} href={m[0]} target="_blank" rel="noopener noreferrer">{m[0]}</a>
          );
          last = m.index + m[0].length;
        }
        if (last < s2.length) nodes.push(<span key={`${keyPrefix}-t${ci}${bi}${ii}-${mi++}`}>{s2.slice(last)}</span>);
      });
    });
  });
  return nodes;
...
export function Markdown({ text }: { text: string }) {
  ...
  while (i < lines.length) {
    ...
    if (h) {
      ...
      blocks.push(<div key={key++} className={`md-h md-h${level}`}>{renderInline(h[2], `h${key}`)}</div>);
      i++;
      continue;
    }
